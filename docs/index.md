---
layout: default
title: index
---

# Nyxera Nexus Docs

Nyxera Nexus integrates SpectraStrike execution assurance with VectorVue continuous validation into a single enterprise evidence plane.

## Structure

- [Introduction](./introduction.md)
- [Solution Overview](./solution-overview.md)
- [Architecture](./nexus-architecture.md)
- [Component Deep Dive](./component-deep-dive.md)
- [Execution to Telemetry Flow](./execution-to-telemetry-flow.md)
- [Federation Model](./federation-model.md)
- [Deployment Patterns](./deployment-patterns.md)
- [Enterprise Security Model](./enterprise-security-model.md)
- [API and Integration](./api-and-integration.md)
- [Data Model](./data-model.md)
- [Threat-Informed Validation](./threat-informed-validation.md)
- [Use Cases](./use-cases.md)
- [Diagrams](./diagrams/README.md)
- [Glossary](./glossary.md)

## Product Documentation

- SpectraStrike Docs: https://docs.spectrastrike.nyxera.cloud
- VectorVue Docs: https://docs.vectorvue.nyxera.cloud
- Nyxera Cloud: https://nyxera.cloud
